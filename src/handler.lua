...
function plugin:access(conf)
    ...
    local function request_access_token(code)
        ...
        local res, err = request:request_uri("https://github.com/login/oauth/access_token", {
            method = "POST",
        ...
        if res.status ~= 200 then
            return nil, "received " .. res.status .. " from https://github.com/login/oauth/access_token: " .. res.body
        end
    ...
    local function request_profile(token)
        ...
        local res, err = request:request_uri("https://api.github.com/user", {
            headers = {
        ...
        if res.status ~= 200 then
            return nil, "received " .. res.status .. " from https://api.github.com/user"
        end
